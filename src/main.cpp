      ...
      if (s != "") {
        ...
        if (event == "telemetry") {
          // j[1] is the data JSON object
           ...
           // Using sensor fusion data to investigate in front car,right and left lane and distances etc.
			for(int i = 0; i < sensor_fusion.size(); i++){
				...
				double check_car_dist = pow(pow(check_car_x - car_x,2)+ pow(check_car_y - car_y,2), 0.5);
				double check_car_dist_s = pow(pow(check_car_dist, 2)- pow(check_car_d - car_d,2), 0.5);
				...
				double time_front_safe = (check_car_dist_s - 10.) / relative_speed;
				double time_back_safe = (check_car_dist_s - -10.) / relative_speed;
				...
				// checking left lane <-------------------
				if((check_car_lane == lane - 1) && (abs(check_car_dist_s) <30)){
					
					if(abs(check_car_dist_s)< 10.){
						left_safe = false;
					}
			...
			} // end of application of sensor fusion data
			...
			// reference x, y, and yaw states. Either refrence from the start point or at the previous path's end point.
			double ref_x = car_x;
			double ref_y = car_y;
			double ref_yaw = deg2rad(car_yaw);


			// if previous size is almost empty, use the car as starting eference.
			if(prev_size <2){
			...
			// using lane distance = 2+4*lane, step_size = 30, number of waypoints = 3
            ...
            // Calculate how to break up the spline so that we travel at the  proper speed.
            ...
            // points. Always output num_points points = 50.
